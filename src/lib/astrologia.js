...
export function longitudeParaSigno(longitude) {
  ...
  return { ...SIGNOS[indice], graus: Number((normalizada % 30).toFixed(1)) }
}

/** Converte hora local no local de nascimento para AstroTime (UTC via fuso IANA ou offset). */
function criarAstroTime(dataISO, horaHHMM, fusoOuLongitude) {
  const fuso = typeof fusoOuLongitude === 'string' || Math.abs(fusoOuLongitude) <= 14
    ? fusoOuLongitude
    : fusoOuLongitude / 15
  const dataUTC = criarDataUTCporLocal(dataISO, horaHHMM, fuso)
  return MakeTime(dataUTC)
}
...
export function calcularMapaNatal({ data, hora, localizacao, fuso = 0 }) {
  ...
  const { asc: lonAsc } = calcularAscendenteEMc(dataUTC, lat, lon)
